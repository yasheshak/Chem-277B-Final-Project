...
Note: Training requires GPU. Estimated training time on CPU is 6+ hours (SchNet) and 20+ hours (DimeNet++).

## Helper Scripts

Each model folder contains its own helper scripts:

- **`read_multi_ase.py`** — Loads `.aselmdb` data files and extracts desired molecule type and amount
- **`extract_ab.py`** — Pre-processes extracted molecules: alpha/beta gap extraction, train/val/test splits, target normalization
